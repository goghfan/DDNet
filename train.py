...
def main():
    ...
    try:
        file = open('train_loss/train_test.txt','w')
        for epoch in range(epochs):
            ...
            for step in tqdm(range(steps_per_epoch)):
                ...
                moving_image = moving_image_origin-diffusion_t1.module.get_feature_extraction(moving_image_origin,t)
                fixed_image = fixed_image_origin-diffusion_t2.module.get_feature_extraction(fixed_image_origin,t)
                # moving_imagee=sitk.GetImageFromArray(moving_image.to('cpu').detach().numpy())
                # fixed_imagee=sitk.GetImageFromArray(fixed_image.to('cpu').detach().numpy())
                ...
                moving_image = torch.cat((moving_image_origin,moving_image.to(device)),dim=1).float()
                fixed_image = torch.cat((fixed_image_origin,fixed_image.to(device)),dim=1).float()
