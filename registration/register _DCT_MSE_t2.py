...
def create_argparser():
    ...
    parser.add_argument('--model',  help='pytorch model for nonlinear registration',default='D:\\Desktop\\diffusion model code\\voxelmorph-dev\model\models_DCT_MSE_t2tot1\\1500.pt')
...
moving_list=glob.glob(os.path.join('data\\train\\T1\\',"*.nii.gz"))[:700]
fixed_list = glob.glob(os.path.join('data\\train\\T2\\',"*.nii.gz"))[:700]
...
for i in tqdm(range(200)):
    ...
    save_file="result\\DCT\\DDPM_DCT_MSE_T2\\"+moving_path[moving_path.find('OAS'):moving_path.find('.nii.gz')]+"_to_"+ fixed_path[fixed_path.find('OAS'):fixed_path.find('.nii.gz')]+".nii.gz"
    deformation_file = "result\\DCT\\DDPM_DCT_MSE_T2\\"+moving_path[moving_path.find('OAS'):moving_path.find('.nii.gz')]+"_to_"+ fixed_path[fixed_path.find('OAS'):fixed_path.find('.nii.gz')]+"_deformation.nii.gz"
    moved, warp = model(fixed_image, moving_image ,registration=True)
    # save moved image
    if args.moved:
